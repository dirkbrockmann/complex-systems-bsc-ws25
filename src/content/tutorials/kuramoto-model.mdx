...
<Margin.Note>
<Margin.Ref id="wurm" />One way of solving this without having to worry about the weird numerical discontinuity 
of the angle $\theta$ in computing averages is going back to Cartesian Coordinates. 
So let's say we have a set of angles $\theta_{n}$, with $n=1,...,N$ and we want to compute 
the average we can form the vectors $(\cos\theta_n,\sin\theta_n)$, compute the averages 
