...
import {each,map,range,shuffle,filter} from "lodash-es";
...
const loadExplorable = (displayContainer,controlsContainer) => {
	...
	const Lx = 600; // world size
	const Ly = 300; // world size
	const dt = 1;
    const R_coll=2;
	const speed = 3;
    const agentsize = 2;
    ...
    const X = d3.scaleLinear().domain([0,Lx]).range([0,config.display.width]);

    const Y = d3.scaleLinear().domain([0,Ly]).range([config.display.height,0]);
    const C = d3.scaleOrdinal().domain(["A","B","C"]).range(["darkred","blue","green"]);
    ...
    function iterate() {
        ...
        const ugg = agents.filter(a=> a.state !== "C");
        each(ugg,a=>{
		    let colliders = [];
		    colliders = agents.filter(function(d){
			    let dx = (a.x-d.x);
			    let dy = (a.y-d.y);
			    return ( Math.sqrt(dx*dx + dy*dy)  < R_coll ) && ( d.state != a.state ) 
		    })
		    each(colliders,b=>b.state="C")
	    })

        agents.forEach(function(d){
		    let phi = g2r(Math.random()*360);
            let speed = d.state== "A" ? sliders[0].value() : sliders[1].value();
		...
		// this takes care of the boundaries
		    ...
		    if (x_new < 0 || x_new > Lx) dx *= -1;
		    if (y_new < 0 || y_new > Ly) dy *= -1;
    ...
    .attr("r",agentsize)
        .style("fill",d=> C(d.state))
        .attr("r",d=> d.state === "C" ? agentsize_C : agentsize)
