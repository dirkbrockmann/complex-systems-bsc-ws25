...
const loadExplorable = (displayContainer,controlsContainer) => {
    ...
    function iterate() {
        ...
        agents.forEach(function(d){
		    ...
		    let dx =  dt * (d.state== "A" ? sliders[0].value() : sliders[1].value()) * Math.cos(phi);
		    let dy =  dt * (d.state== "A" ? sliders[0].value() : sliders[1].value()) * Math.sin(phi);
		    let x_new= (d.x + dx);
		    let y_new= (d.y + dy);
    ...
    function setup() {
        ...
        display.selectAll("." + styles.agent).data(agents,d=>d.id).enter().append("circle")
            ...
            .attr("r",agentsize)
            .style("fill",d=> d.state=="A" ? "darkred" : "blue")
            .attr("transform",function(d) {return "translate(" + X(d.x) + "," + Y(d.y) + ")"})
