/* Diffusion or particles in two dimensions */

import * as d3 from 'd3';
import * as widgets from 'd3-widgets';
import {filter,each,map,meanBy,sumBy,shuffle,sample,sampleSize} from "lodash-es";
import {useEffect,useRef} from 'react';
import config from './config.js';
import styles from './styles.module.css';
import {square} from "lattices"
import {torusdist,grid,l2d,sigmoid} from "./utils.js"


const loadExplorable = (displayContainer,controlsContainer) => {

    const display = d3.select(displayContainer.current)
    const controls = d3.select(controlsContainer.current)

	const L = 100; // world size
	const N = 100;
    const active_tail = 10;
    var eq_events = 0;
    const W = config.display.width;
    const H = config.display.height;

    const X = d3.scaleLinear().domain([-N,N]).range([0,W]);;
    const Y = d3.scaleLinear().domain([-N,N]).range([0,H]);;
    const C = d3.scaleLinear().domain([0, 3]).range(["black", "white"])
	const C_active = d3.scaleLinear().domain([0, 5]).range(["black","red"])
	
	const ctx = display.node().getContext('2d');	
	ctx.clearRect(0, 0, W, H);

    const s = square(N).scale(2*N).hood("n4").boundary("dirichlet");
    var agents = s.nodes;
	var avalanche_size = 0;
    var timer = {}
    
    var tick, agents;

    const g = widgets.grid(config.controls.width,config.controls.height,config.controls.grid.x,config.controls.grid.y);

        // controls.selectAll("*").data(g.points).enter().append("circle")
        // .attr("class","grid")
        // .attr("transform",function(d) {return "translate(" + d.x + "," + d.y + ")"})
        // .attr("r",1)
        // .style("fill","black")
        // .style("stroke","none")

    const buttons = map(config.widgets.buttons,
        v => widgets.button()
            .id(v.id)
            .value(v.value)
            .size(v.size)
            .actions(v.actions)
            .position(g.position(v.position.x,v.position.y))
    );

    const sliders = map(config.widgets.sliders,
            v => widgets.slider()
                .id(v.id)
                .label(v.label)
                .range(v.range)
                .value(v.value)
                .fontsize(config.widgets.slider_fontsize)
                .labelpadding(0)
                .size(config.widgets.slider_size)
                .girth(config.widgets.slider_girth)
                .knob(config.widgets.slider_knob)
                .position(g.position(v.position.x,v.position.y))
        );
    
    function paint_single(agent){
            const c = agent.cell();		    
		    const color = agent.avalance ? "red" : C(agent.state);
		    ctx.fillStyle=color;
		    ctx.lineWidth = 0;
		    ctx.fillRect(X(c[0].x),X(c[0].y),X(c[2].x)-X(c[0].x),X(c[2].y)-X(c[0].y))
    }
    function paint(){
        agents.forEach(d=>{
		    const c = d.cell();		    
		    const color = d.avalance ? "red" : C(d.state);
		    ctx.fillStyle=color;
		    //ctx.strokeStyle=color;
		    ctx.lineWidth = 0;
		    ctx.fillRect(X(c[0].x),X(c[0].y),X(c[2].x)-X(c[0].x),X(c[2].y)-X(c[0].y))
	    })	
    }
    // single relaxation step: let all currently active agents topple once
    function relaxOnce() {
        const active = filter(agents, a => a.state > 3);
        

        each(active, a => {
            a.avalance = true
            a.state -= 4;
            each(a.neighbors, b => {
                b.state += 1;
            });
        });
        avalanche_size+=active.length;
        return active.length;
    }

    // keep relaxing until no active agents remain
    function equilibrate() {
        eq_events+=1;
        avalanche_size=0;
        // simple while loop; for safety you could guard with an iteration cap
        while (relaxOnce() > 0) {
            // nothing else needed; relaxOnce mutates agents in place
        }
        paint();
        console.log(eq_events,avalanche_size);
    }

    function iterate() {
        tick++;
        
	    ctx.clearRect(0, 0, W, H);
        
        const centralpatch = filter(agents,a=>{
            return a.x*a.x+a.y*a.y<sliders[1].value()*sliders[1].value();
        });

        // pick up to 10 random agents from centralpatch
        const randomAgents = sampleSize(centralpatch, 1);
        randomAgents.forEach(a => {
            a.state += 1;
        });

        // fully relax until no more active sites
        equilibrate();

        agents.forEach(d=>{d.avalance = false});
        //paint()
	    
        
       
    }

    function update(){
         agents.forEach(d=>{
		    const c = d.cell();
		    const color = C(d.state);
		
		    ctx.fillStyle=color;
		    ctx.strokeStyle=color;
		    ctx.lineWidth = 0;
		    ctx.fillRect(X(c[0].x),X(c[0].y),X(c[2].x)-X(c[0].x),X(c[2].y)-X(c[0].y))
	    })	
    }

    function setup() {
        
        tick = 0;

        each(agents,a=>{
		    a.state=0;
            a.memory=0
	    })

        var centralpatch = filter(agents,a=>{
            return a.x*a.x+a.y*a.y<sliders[1].value()*sliders[1].value();
        });
        
        each(centralpatch,a=>{
            // random integer in {0,1,2,3}
            a.state = 1+Math.floor(Math.random() * 3);
        });
        ctx.clearRect(0, 0, W, H);
        paint();
 
    }

    function go(d) {
        //const interval = ; // ms between calls to iterate
        if (buttons[0].value() == 1) {
            let last = 0;
            timer = d3.timer((elapsed) => {
                if (elapsed - last >= sliders[0].value()) {
                    last = elapsed;
                    iterate();
                }
            }, 0); // start immediately
        } else {
            timer.stop();
        }
    }

    buttons[0].update(go)
    buttons[1].update(setup)
    sliders[1].update_end(setup)
    
    controls.selectAll(null).data(buttons).enter().append(widgets.widget);
    controls.selectAll(null).data(sliders).enter().append(widgets.widget);
   
    setup();
    
    return () => {
        controls.selectAll("*").remove(); // Remove all circles from the second SVG
        display.selectAll("*").remove(); // Remove all circles from the second SVG
    };
}

export default ({id}) => {
    const displayContainerRef = useRef(null); // Ref for the first div
    const controlsContainerRef = useRef(null); // Ref for the second div
    const ContainerRef = useRef(null); // Ref for the first div

    useEffect(() => {

        return loadExplorable(displayContainerRef,controlsContainerRef);


    },[id]); // Add `id` as a dependency to ensure it updates if the prop changes

    return (
        <>
                    <div
                        ref={ContainerRef}
                        id={id}
                        className={config.container.class}
                    >
                        <canvas
                            ref={displayContainerRef}
                            id={`${id}-display`}
                            className={config.display.class}
                            width={config.display.width}
                            height={config.display.height}
                        />
                        <svg
                            ref={controlsContainerRef}
                            id={`${id}-controls`}
                            className={config.controls.class}
                            
                            viewBox={`0 0 ${config.controls.width} ${config.controls.height}`}
                        />
                    </div>
                </>
    );
}