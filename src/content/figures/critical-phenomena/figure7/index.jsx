...
const loadExplorable = (displayContainer,controlsContainer) => {
    ...
    const df = (x,alpha,beta,H,sigma,dt) => dt*alpha*x*x*(1-x) - dt*(beta+H)*x + Math.sqrt(sigma*dt)*x*(1-x)*dw();
    ...
    const iterate = () => { 
        ...
        each(agents,a => {
            ...
            let H = sliders[0].value();
            let sigma = sliders[1].value();
            a.trajectory.push({t: t + dt,X: last(a.trajectory).X + df(last(a.trajectory).X,alpha,beta,H,sigma,dt)});
        });
    ...
    const reset = () => { 
        ticks = 0
        agents = map(range(N),(d,i) => ({id: i,trajectory: [{t: 0,X: 0.1+0.9*Math.random()}]}))
    ...
    const H0 = config.bifurcation_plot.xr[0];
    const H1 = config.bifurcation_plot.xr[1];
    const H = range(H0, H1, dx);   
    
    each(branches,branch => {        
        const [xmin, xmax] = branch.xr;
        const f = branch.f(beta); // ensure f has correct signature elsewhere
        const a = H.filter(x => x >= xmin && x <= xmax);
