...
const loadExplorable = (displayContainer,controlsContainer) => {
    ...
    const f = (x) => 
        {
            return a.value()*x*x*(1-x)-b.value()*x-H.value()*x;
        };
    const fp = (x) => {
        return a.value()*2*x*(1-x) + a.value()*x*x*(-1) - b.value()-H.value();
    };
