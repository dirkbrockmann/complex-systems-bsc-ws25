...
const loadExplorable = (displayContainer,controlsContainer) => {
    ...
    const f = (x) => {return a.value()+b.value()*x+c.value()*0.5*x*x+d.value()*x*x*x/6};
    const fp = (x) => {return b.value()+c.value()*x+d.value()*x*x/2};
    ...
    display.append("text").text(config.plot.xaxis.label).attr("class",styles.axis_label)
        .attr("transform","translate(" + X(4.5) + "," + (Y(0) + 40) + ")")
