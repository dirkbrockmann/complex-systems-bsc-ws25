...
const loadExplorable = (displayContainer,controlsContainer) => {
    ...
    function iterate() {
        ...
        T.domain([tick>Tmax?tick-Tmax:0,tick>Tmax?tick:Tmax])
        R.domain(d3.max(count,x=>x.r)>config.plot.yrange[1]?[0,1.1*d3.max(count,x=>x.r)]:config.plot.yrange)
        
        controls.select("." + styles.xaxis).call(tAxis)
        controls.select("." + styles.yaxis).call(rAxis)
