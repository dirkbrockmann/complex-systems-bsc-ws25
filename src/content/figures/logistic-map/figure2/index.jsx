...
const loadExplorable = (displayContainer,controlsContainer) => {
    ...
    var points = map(data,(d,i) => { return {x: i, y: d}});
    
    const update = () => {
        data = logistic(x0.value(),lambda.value(),N);
        points = map(data,(d,i) => { return {x: i, y: d}});
	...
	display.selectAll(".dots").data(points).enter().append("circle")
				.attr("r",config.plot.dotsize).attr("class",styles.dots)
						.attr("transform",function(d){return "translate("+X(d.x)+","+Y(d.y)+")"})
