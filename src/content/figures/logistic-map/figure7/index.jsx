...
const loadExplorable = (canvasContainer,SvgContainer) => {
    ...
    display.append("text").text(config.plot.yaxis.label)
        .attr("class",styles.axis_label)
        .attr("transform","translate(" + (X(config.plot.xr[0]) + config.plot.yaxis.label_position.x) + "," + (Y(0) + config.plot.yaxis.label_position.y) + ")rotate(-90)")
