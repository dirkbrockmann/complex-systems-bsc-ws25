...
const loadExplorable = (canvasContainer,SvgContainer) => {
    ...
    const f = (x,r) => { return r*x*Math.exp(-x);};
    ...
    d3.select(canvas)
        ...
        .on("mouseover",function(){
		    d3.selectAll("."+styles.fadenkreuz).transition().duration(100).style("opacity",1)
		    d3.selectAll("."+styles.coord).transition().duration(100).style("opacity",1)
	    })
	    .on("mouseout",function(){
		    d3.selectAll("."+styles.fadenkreuz).transition().duration(100).style("opacity",0)	
		    d3.selectAll("."+styles.coord).transition().duration(100).style("opacity",0)
	    })
        .on("mousemove", function() {
		    ...
		    d3.select("#vertical").attr("x1",X(lamm)).attr("x2",X(lamm))
		    d3.select("#horizonal").attr("y1",Y(xm)).attr("y2",Y(xm))
            
            d3.select("."+styles.coord).datum(p).attr("transform",function(d){
    ...
    const display = d3.select(svg).append("g")
    ...
    display.append("text").text(config.plot.yaxis.label)
        .attr("class",styles.axis_label)
        .attr("transform","translate(" + (X(config.plot.xr[0]) + config.plot.yaxis.label_position.x) + "," + (Y(6) + config.plot.yaxis.label_position.y) + ")rotate(-90)")
