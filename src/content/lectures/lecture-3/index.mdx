---
title: 'One-dimensional continuous dynamical systems'
description: 'In this lecture we will explore how to develop one-dimensional dynamical systems to model the behavior of simple systems and how to analyse them with a lot of thought and little math.'
...
import Fig1 from '../../../content/figures/1-dimensional-dynamical-systems/figure1';
import Fig2 from '../../../content/figures/1-dimensional-dynamical-systems/figure2';
import Fig3 from '../../../content/figures/1-dimensional-dynamical-systems/figure3';
import Fig4 from '../../../content/figures/1-dimensional-dynamical-systems/figure4';
import Fig5 from '../../../content/figures/1-dimensional-dynamical-systems/figure5';
import Fig6 from '../../../content/figures/1-dimensional-dynamical-systems/figure6';
import dynsys_fig1  from '../../../assets/generated/dynsys_fig1_transparent.png';
import dynsys_fig1Dark from '../../../assets/generated/dynsys_fig1_dark.png';
import dynsys_fig3  from '../../../assets/generated/dynsys_fig3_transparent.png';
import dynsys_fig3Dark from '../../../assets/generated/dynsys_fig3_dark.png';


<Margin.Note>
If you want to revisit the slides, you will be able to check out the [**Lecture 3 - Slidedeck**](slides/slide-1-one-dimensional-dynsys/).
The slides only contain the interactive viszualizations that were used in class.
</Margin.Note>

In the last lecture on the [Logistic Map](../lecture-2/), we discussed properties of one-dimensional discrete time maps, defined by the iteration rule:
$$
x_{n+1}=f(x_{n})
$$

with initial condition $x_{0}$ and evolving in discrete steps $n=0,1,2,...$. We discovered that, for example, in spite of its simplicity the
logistic map
...
<Explorable id="figure_1" margin={false}>    
    **A particle annihilation system:** Initially we have $x(t_0)$ particles in a box. 
    When they collide they annihilate. What is the expected time course of $x(t)$?
    <Fig1 slot="figure" client:load id="annihilation"/>
</Explorable>

### Example: Particles in a box that annihilate on collision

Let's use a simple example to explain. Assume that we have a
container of molecules of type $A$. When two molecules collide
they annihilate and disappear like so:
$$
A+A\xrightarrow{\alpha}\emptyset
$$

The parameter $\alpha>0$ is a reaction rate that quantifies how likely
collisions yield an annihilation event.
...
Let's also assume that the number of molecules is large, so that it makes
sense to say something like 5% of the particles but that if we
remove or add one particle is reflects really an infinitesimal change.
...
way $x(t)$ may decrease over time. Will it decrease exponentially? Algebraically? If so what is the exponent?

We can develop a model based on a few plausible assumptions.
...
proportionality constant is $f(x(t))$ which can depend on the state $x(t)$
that we are in. This naturally yields
...
have been made in order for these types models to be valid. Therefore:
...
to the abundance or density of the molecules altogether. So the expected change in the
abundance caused by a _single molecule_ $i$ is


<Figure src={dynsys_fig1} srcDark={dynsys_fig1Dark} alt="Exponential Growth." margin={true}>
The solution $x(t)$ to the ODE model for a population of particles that annihilate. 
Here we have $x(0)=1200$ and $\alpha=0.01$
</Figure>
...
and integrating<Margin.Cite id="integration" />. 

<Margin.Note>
<Margin.Ref id="integration" />We have to integrate both sides of the equation. Clearly 
$$
\int_{t_0}^{t_1} dt = t_1 - t_0.
$$
Integrating the left side:

$$
-\frac{1}{\alpha}  \int_{x_0}^{x_1} \frac{dx}{x^2} = \frac{1}{\alpha x(t)}-\frac{1}{\alpha x(t_{0})}.
$$
...
A\rightarrow 2A.
$$
We can see already that the second reaction can generate new molecules, because a single particle
can spontaneously replicate.
...
other which is captured by the first equation. Below is a simulation of this system.
 ...
 <Explorable id="figure_2" margin={false}>    
    **Reproduction and annihilation:**
    Here's a system of particles in a box. Inititally, 
    we have a few particles at low concentration. These particles 
    can replicate at a specified reproduction rate. When they 
    collide they annihilate. You can vary the these parameters with 
    the sliders. Start by pressing play and wait a bit until 
    the particle number increases. When the density is 
    sufficiently high, particles will start to annihilate. 
    Eventually an equilibrium is reached.
    <Fig2 slot="figure" client:load id="annihilation"/>
</Explorable>

We are going to use the letter '$n$' to quantify the number of particles, so the
state of the system is described by $n(t)$<Margin.Cite id="noft" />. 

<Margin.Note>
<Margin.Ref id="noft" />We need the letter '$x$' later, so that's why we'll use $n$ here.
Sometimes the right choice of letters is important.
</Margin.Note>
...
in $n(t)$ is proportional to $n(t)$ and positive
$$
\Delta n=\beta n \Delta t
$$
where $\beta$ is a proportionality constant. In combination with
the annihilation reaction this yields the system
$$
\dot{n}=\beta n-\alpha n^{2}
$$
which we write as
$$
\dot{n}=n(\beta-\alpha n)
$$

Now let's recall what we are modeling here. $n(t)$ is an abundance
or concentration so it cannot be negative, i.e. $n(t)\geq0$. The
...
equation<Margin.Cite id="integral_logistic" />. 

<Margin.Note>
<Margin.Ref id="integral_logistic" />Integrating yields the solution:

$$
n(t)=\frac{\frac{\beta}{\alpha}}{1 + \left( \frac{\beta}{\alpha n(t_{0})} - 1 \right)e^{-\beta (t - t_{0})}}.
$$

</Margin.Note>

$$
\int_{n(t_{0})}^{n(t)}\frac{dn}{n(\beta-\alpha n)}=\int_{t_{0}}^{t}dt
$$
which is 
...
We can understand what's going on without the math. 
Let's look at the dynamical system again:

$$
\dot{n}=n(\beta-\alpha n).
$$

In what follows we set the beginning of time $t_{0}=0$ and have $n(0)=n_{0}\geq0$
as the initial condition. If $n_{0}=0$ then $\dot{n}=0$ which implies
that there is no change in $n(t)$. If $n(0)=0$ then $n(t)=0$ for
all times. Likewise, if $n_{0}=\beta/\alpha$ we have $\dot{n}=0$
and $n(t)=\beta/\alpha$ for all times. 

Let's do a little trick before we proceed. Let's define a new variable
$$
x(t)=\frac{\alpha}{\beta}n(t)
$$
which is just a scaled version of $n(t)$. We can derive the differential 
equation fort $x(t)$, and you can check that this is:
$$
\dot{x}=\beta x(1-x).
$$

That looks pretty much like the logistic map, right? But note that
we don't a a discrete time map, but a differential equation. This makes
a big difference as you'll see. In the above equation
the point $x=0$ and $x=1$ are special because they yield $\dot{x}=0$, which
...
Let's go back to the original variable $n(t)$.

Now, the ODE system above has two fixpoints as mentioned above:
$$
n_{1}^{\star}=0
$$
and
$$
n_{2}^{\star}=\frac{\beta}{\alpha}>0
$$
If we start at any value except these two points we will find that
every initial condition yields a trajectory 
$$
n(t)\rightarrow \frac{\beta}{\alpha}
$$
...
<Explorable id="logistic_growth" margin={true}>    
    **Graphical analysis of the model:**
    We plot the function $f(n)$.
    Fixpoints are zeros of $f(n)$ because that implies $\dot n = 0$.
    When $f(n)>0$ the change $\dot n$ is positive, so $n(t)$ increases.
    When $f(n)< 0$ the opposite is happening. This change is indicated
    by the direction of the arrows on the $x$-axis. That implies: the red fixpoint ($n^\star=0$) is unstable, 
    the blue one ($n^\star=\beta/\alpha$) is stable. 
    <Fig3 slot="figure" client:load id="logistic_growth"/>
</Explorable>

This is a parabola that opens downwards and crosses the $n$-axis
at the fixpoints. Between the fixpoints, in the interval $ (0,\beta / \alpha) $,
the parabola $f(n)>0$. Whenever we have an $n(t)$ in that range
it will increase until it reaches the fixpoint $n^{\star}=\beta/\alpha$.
If on the other we start on the right of that fixpoint the parabola
is negative so the change in $n(t)$ is negative and $n(t)$ decreases.
...
easily by just plotting the function $f(n)$ and observing where it
is positive or negative and indicate this by arrows to the right and
left, respectively on the $n$-axis. 
...
We also see an important defining feature of stability. 
<Margin.Note>
A small note about notation. In dynamical systems theory and other fields
it is customary to denote the derivative of something with respect to time
with a dot, so
$$
dx/dt = \dot x.
$$
whereas the derivative with respect to the generic argument of a function is
denoted by a prime so for instance
$$
df(x)/dx=f^\prime(x).
$$
It's confusing at times. But it's really usually clear from the context what
is meant.

</Margin.Note>
>A stable fixpoint
is one in which the derivative of $f(x)$ is negative at the fixpoint
whereas when $f^{\prime}(x)$ is positive, the fixpoint is unstable. 
...
<Explorable id="qubic" margin={false}>    
    **Graphical Analysis: Finding fixpoints and checking their stability:**
    The function $f(x)=a+bx+cx^2/2+dx^3/6$. You can vary 
    the coefficients $a$,$b$,$c$ an $d$. 
    For different values the system can have different 
    sets of fixpoints with different stability properties.
    <Fig4 slot="figure" client:load id="qubic"/>
</Explorable>
...
<Figure src={dynsys_fig3} srcDark={dynsys_fig3Dark} alt="Exponential Growth." margin={true}>
**Marginal Stability:** This happens when $f(x)=f^\prime(x)=0$.
</Figure>
...
become susceptible again. In mathematical epidemiology this is known as the **SIS model**
( Susceptible - Infected - Susceptible).

The panel below depicts a simulation that
captures such a scenario.

 <Explorable id="epidemic" margin={false}>    
    **An epidemic model:**
    This is an implementation of the SIS model. 
    Infected individuals are shown in red. When they encounter 
    a susceptible (white) an infection can occur. 
    Infecteds recover at the recovery rate. If the 
    transmission rate is too low or the recovery rate is too high, 
    the disease dies out.
    <Fig5 slot="figure" client:load id="epidemic"/>
</Explorable>

Let's try to develop a set of mathematical equations, i.e. differential equations that
...
in each group by $S(t)$ and $I(t)$ respectively. 
...
interval $t,t+\Delta t$ each infected person can infect a susceptible person which
increases the number of infected individuals. This change should be proportional
to the number of infected individuals $I(t)$ because each of them can transmit the disease.
The increase should also be proportional to 
the fraction of susceptible $S(t)/N$ because if a contact occurs an infection only occurs if the other person is susceptible, 
not if the other person is also infected. The increment due to the first reaction is therefore
$$
\Delta I_{1}=\alpha IS/N\Delta t
$$
Likewise, the number of infected individuals can decrease because each infected person
...
$$
This almost looks like the logistic equation we discussed before. But it's
not quite the same. Let's analyze the fixpoints of this system.
...
If $R_{0}< 1$ the disease free state is the only stable fixpoint. 

We can confirm this analytically if we look that the derivative that the fixpoints:
$$
f^{\prime}(x)=\alpha(1-2x)-\beta
$$
at $x^{\star}=0$ we have
$$
f^{\prime}(0)=\alpha-\beta=\beta(R_{0}-1)
$$
which is negative if $R_{0}< 1 $ and positive if $R_{0} > 1$. 
At the second fixpoint we have
$$
f^{\prime}(1-1/R_{0})=-\alpha+\beta=-\beta(R_{0}-1)
$$
which is positive if $R_{0}< 1$ and negative if $R_{0}>1$. So we have confirmed our graphical analysis.  

<Explorable id="sis_fofx" margin={false}>    
**Graphical Analysis of the SIS model:** Only if $$\alpha > \beta$$ (i.e. $$R_0>1$$) the epidemic becomes endemic,
which means the non-trivial fixpoint is stable and the disease free state is stable. 
    <Fig6 slot="figure" client:load id="sis_fofx"/>
</Explorable>


An important feature of this model is precisely this threshold property. 
The final state of the system is different for different values of $R_0$. 
The basic reproduction number has to be larger than $1$ in order for the epidemic to become endemic, 
that is, prevail in the population.
...
In both systems we have reactions that drive the proliferation of a type of particle or individual, 
like the reproduction in the first system and the infection the epidemic model, 
and in both systems we have a self-regulation that inhibits proliferation, like the annihilation reaction in 
the first system and the recovery in the epidemic system. 
However, in the first system the proliferation is caused by a single particle 
and regulation requires an interaction. 
Whereas in the epidemic model the proliferation requires two particles and the regulation only one.

This yields similar dynamical systems (quadratic functions $f(x)$). However, 
they are quite different in the dynamics that they induce. 
The epidemic model exhibits different asymptotics depending on the ratin $\alpha/\beta$. 
In one regime the epidemic dies out and only for sufficiently large $R_0$ is prevails. 
Whereas in the reproduction annihilation model, a stable equilibrium is always reached, as long
as both parameters are positive
...
Let's assume that we have a situation in which $x(t)$
...
in the solution to the ODE so we look at:
...
We then define
$$
x_{n+1}=x_{n}+\Delta tf(\bar{x}_{n})
$$
...
called the RK4. The idea is just like the second order RK,
